/**
 * AV-Master-Periscope-GoogleSheets - Apps Script Web App
 *
 ...
 *          whose A1 reads mission_sas_id (named "DATA" in the older files),
 *          48 columns, header row below.
 *
 * Contract with the scraper (scrape_and_upload.py):
 *   GET  ?token=...                -> {success:true} health check
 *   POST ?token=...  {"rows":[[...48 cols...], ...]}
 ...
 *        Every write goes through setNumberFormat('@') first so Sheets
 *        never locale-parses "09/03/2026" into a Date.
 *
...
var TZ = 'America/Bogota';         // only used when a date cell is a real Date

// Header row exactly as it exists in the monthly files today (48 columns,
// including a duplicated "task_15" and a trailing "USU_MES"). The scraper
// maps the report's 46 CSV columns onto this header by name before posting.
var HEADERS = [
  ...
  'task_15', // duplicated in the existing files' header - kept so columns line up
...
// GET ?token=...&action=restore_text&file=9_2026_AV
//                                     -> one-off repair (see debugRestoreTextCells)
// GET ?token=...&action=rebuild&file=9_2026_AV
...
// The data lives in the tab whose A1 is mission_sas_id ("DATA" in the older files).
...
// One-off repair helper (inherited from the CM pipeline). A plain setValues
// on automatic-format cells lets Sheets parse text dates/datetimes/numbers into real Date/number cells
...
var DATETIME_COLS = [14, 15, 19, 20, 21]; // arr_time, dep_time, assign_time, start_time, finish_time
function debugRestoreTextCells(fileName) {
  fileName = fileName || '9_2026_AV';
  var folder = DriveApp.getFolderById(FOLDER_ID);
